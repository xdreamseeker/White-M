...
function themeConfig($form) {
    ...
    $form->addInput($qqUrl);
    /** 缩略图显示 */
    $thumbDisplay = new Typecho_Widget_Helper_Form_Element_Radio('thumbDisplay', array(
        'display' => _t('显示') ,
        'close' => _t('关闭')
    ) , 'close', _t('显示缩略图') , _t('请将主题目录下的Cache文件夹设置为777.<br /><span style="color:red;font-weight:bold;">SAE或BAE等不兼容timthumb的用户请保持禁用此功能！</span>'));
    $form->addInput($thumbDisplay);
    /** RSS开关 */
    $rssDisplay = new Typecho_Widget_Helper_Form_Element_Radio('rssDisplay', array(
        'display' => _t('显示') ,
        'close' => _t('关闭')
    ) , 'display', _t('显示RSS订阅和相关文章'));
    ...
    $pattern = new Typecho_Widget_Helper_Form_Element_Radio('pattern', array(
